...
2- If running PyTestUtil via KCS, the following package needs to be
   installed:
...
6- Open command prompt and cd to PyTestUtil root directory
7- Run for usage help: python PyTestUtil.py -h

---- Example: Running tests via IPMI over LAN+ ----

1- Copy PyTestUtil to a machine that will remotely test 
...
8- Open command prompt and cd to PyTestUtil folder
9- Run all functional tests: 
   python PyTestUtil.py -conn eth -ip <IpAddress> -user <BmcUser> -pwd <BmcPassword> -test a

---- Example: Running tests via KCS ----

1- Copy PyTestUtil to compute server blade
  - NOTE: Python 2.7 must be installed as instructed in 
    "Getting Started" section
2- Open command pormpt and cd to PyTestUtil folder
3- Run all functional tests:
   python PyTetsUtil.py -conn kcs -test a
...
6- Verify BMC dynamic IP address is set by pinging from machine
   that will run PyTestUtil ("ping <IpAddress>")
7- Note the BMC username (<BmcUser>) and BMC password (<BmcPassword>)
   that can be used to run REST commands on J2010 BMC
8- Open command prompt and cd to PyTestUtil folder
9- Run all J2010 tests: 
   python PyTestUtil.py -plt J2010 -conn eth -ip <IpAddress> -user <BmcUser> -pwd <BmcPassword> -test a
